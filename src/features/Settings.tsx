...
import { User, Palette, Bell, Database, Zap, Shield, Sun, Moon, RefreshCw, Cloud, CloudOff, LogIn, LogOut, Mail, Lock, CheckCircle, AlertCircle } from 'lucide-react';
...
function AuthForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (isSignUp) {
        ...
        addToast({ message: 'Account created! Check your email to confirm.', type: 'success' });
      } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      {error && (
        <p className="flex items-center gap-2 font-mono text-xs text-[#fa7a7a]">
          <AlertCircle size={12} />
          {error}
        </p>
      )}
      <div className="space-y-1">
        <label className="font-mono text-xs uppercase tracking-wide text-on-surface-variant dark:text-[#777584]">Email</label>
        <div className="relative">
          <Mail size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant dark:text-[#777584]" />
          <input
            type="email"
            value={email}
            ...
            required
            className="w-full pl-9 pr-3 py-2 border-2 border-on-surface dark:border-[#a8a6ff] bg-surface dark:bg-[#252533] text-on-surface dark:text-[#e5e1ea] font-body text-body-sm focus:outline-none focus:border-[var(--color-primary-fixed-dim-light)]"
          />
        </div>
      </div>
      <div className="space-y-1">
        <label className="font-mono text-xs uppercase tracking-wide text-on-surface-variant dark:text-[#777584]">Password</label>
        <div className="relative">
          <Lock size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant dark:text-[#777584]" />
          <input
            type="password"
            value={password}
      ...
      <div className="flex items-center gap-3 pt-1">
        ...
        <button
          ...
          className="font-mono text-xs text-primary dark:text-[var(--color-primary-fixed-dim-dark)] underline hover:no-underline min-h-[32px]"
        >
...
export function SettingsPage() {
  const { settings, updateSettings, theme, addToast, user, isAuthenticated, syncStatus, syncNow, signOut, syncResult } = useApp();
          ...
          {activeTab === 'profile' && (
            <SettingsSection title="Profile" description="Your personal information">
              <div className="flex items-center gap-5 pb-4 border-b border-on-surface/10 dark:border-[#464552]/50">
                ...
                <div>
                  <p className="font-headline font-semibold text-on-surface dark:text-[#e5e1ea]">{settings.name}</p>
                  <p className="font-mono text-xs text-on-surface-variant dark:text-[#777584]">{settings.email}</p>
                ...
                {isAuthenticated ? (
                  ...
                  <div className="space-y-4">
                    ...
                    {showAuth ? (
                      ...
                      <Button variant="primary" icon={<LogIn size={14} />} onClick={() => setShowAuth(true)}>
                        Sign In with Email
                      </Button>
                    )}
                  </div>
